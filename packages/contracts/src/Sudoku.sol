...
contract Sudoku {
    ...
    // !TODO: add 3 boards into a single uint256 (3 * 9 * 9 * 1 byte (0 - 8) = 243 bytes)
    uint8[9][9][3] private BOARD_LIST = [
        [
            [1, 2, 7, 5, 8, 4, 6, 9, 3],
            [8, 5, 6, 3, 7, 9, 1, 2, 4],
            [3, 4, 9, 6, 2, 1, 8, 7, 5],
    ...
    function verifyProof(
        ...
        returns (bool)
    {
        return ISudokuVerifier(verifierAddress).verifyProof(a, b, c, input);
    }
    ...
    function verifySudoku(
        ...
        returns (bool)
    {
        require(verifySudokuBoard(board), "This board does not exist");
        require(verifyProof(a, b, c, board), "Filed proof check");
        return true;
    }
    ...
    function pickRandomBoard(string memory stringTime) private view returns (uint8[9][9] memory) {
        ...
        return BOARD_LIST[randPosition];
    }

    function verifySudokuBoard(uint256[81] memory board) private view returns (bool) {
        bool isEqual = true;
        for (uint256 i = 0; i < BOARD_LIST.length; ++i) {
            isEqual = true;
            for (uint256 j = 0; j < BOARD_LIST[i].length; ++j) {
                for (uint256 k = 0; k < BOARD_LIST[i][j].length; ++k) {
                    if (board[9 * j + k] != BOARD_LIST[i][j][k]) {
                        isEqual = false;
                        break;
                    }
                }
